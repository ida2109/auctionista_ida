...
--
-- Host: localhost
-- Generation Time: Mar 19, 2020 at 02:55 PM
...
(1, 'Pediatric Infuvite Multiple Vitamins for Infusion', 47, 'Dilation of Innom Art with 2 Drug-elut, Perc Approach', 'http://dummyimage.com/136x245.png/dddddd/000000', 1, '2020-03-17 00:00:00', '2020-05-17 00:00:00'),
(2, 'Imipramine Hydrochloride', 54, 'Insert Infusion Dev in Uterus & Cervix, Perc Endo', 'http://dummyimage.com/188x235.bmp/5fa2dd/ffffff', 251, '2020-03-17 00:00:00', '2020-05-30 00:00:00'),
...
(5, 'Arnica Betula A', 13, 'Removal of Drain Dev from R Sacroiliac Jt, Perc Approach', 'http://dummyimage.com/210x248.bmp/ff4444/ffffff', 3, '2020-03-17 00:00:00', '2020-05-17 00:00:00'),
...
(9, 'AILANTHUS ALTISSIMA POLLEN', 93, 'Plain Radiography of Right Parotid Gland using Oth Contrast', 'http://dummyimage.com/140x150.png/5fa2dd/ffffff', 77, '2020-03-17 00:00:00', '2020-05-03 00:00:00'),
...
(11, 'Grain Smut Mixture', 86, 'Supplement Left Humeral Shaft with Nonaut Sub, Perc Approach', 'http://dummyimage.com/178x212.jpg/cc0000/ffffff', 512, '2020-03-17 00:00:00', '2020-05-17 00:00:00'),
(12, 'Instant Hand Sanitizer Original With Moisturizer and Vitamin E', 85, 'Removal of Radioactive Element from Right Eye, Via Opening', 'http://dummyimage.com/215x154.png/5fa2dd/ffffff', 1, '2020-03-17 00:00:00', '2020-05-17 00:00:00'),
...
(22, 'Gentamicin', 12, 'Division of Tibial Nerve, Open Approach', 'http://dummyimage.com/154x116.bmp/ff4444/ffffff', 9813, '2020-03-17 00:00:00', '2020-05-17 00:00:00'),
(23, 'DIVALPROEX SODIUM', 69, 'Drainage of Right Thorax Muscle, Perc Endo Approach, Diagn', 'http://dummyimage.com/210x156.bmp/5fa2dd/ffffff', 0, '2020-03-17 00:00:00', '2020-05-17 00:00:00'),
...
--
-- Stand-in structure for view `User story 3 (alla pågående auktioner)`
-- (See below for the actual view)
--
CREATE TABLE `User story 3 (alla pågående auktioner)` (
...
--
-- Stand-in structure for view `User story 5 (ser vem som är säljare)`
-- (See below for the actual view)
--
CREATE TABLE `User story 5 (ser vem som är säljare)` (
...
--
-- Stand-in structure for view `User story 6 (senaste budet först)`
-- (See below for the actual view)
--
CREATE TABLE `User story 6 (senaste budet först)` (
...
--
-- Structure for view `User story 3 (alla pågående auktioner)`
--
DROP TABLE IF EXISTS `User story 3 (alla pågående auktioner)`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `User story 3 (alla pågående auktioner)`  AS  select `items`.`item_id` AS `item_id`,`items`.`item_name` AS `item_name`,`items`.`seller_id` AS `seller_id`,`items`.`item_desc` AS `item_desc`,`items`.`item_img` AS `item_img`,`items`.`reserved_price` AS `reserved_price`,`items`.`auction_start` AS `auction_start`,`items`.`auction_end` AS `auction_end` from `items` where `items`.`auction_end` >= '2020-03-20 14:00:00' ;

-- --------------------------------------------------------

--
-- Structure for view `User story 5 (ser vem som är säljare)`
--
DROP TABLE IF EXISTS `User story 5 (ser vem som är säljare)`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `User story 5 (ser vem som är säljare)`  AS  select `users`.`user_name` AS `user_name`,`items`.`item_id` AS `item_id`,`items`.`item_name` AS `item_name`,`items`.`item_desc` AS `item_desc`,`items`.`item_img` AS `item_img`,`items`.`reserved_price` AS `reserved_price`,`items`.`auction_start` AS `auction_start`,`items`.`auction_end` AS `auction_end` from (`items` join `users`) where `items`.`seller_id` = `users`.`user_id` ;

-- --------------------------------------------------------

--
-- Structure for view `User story 6 (senaste budet först)`
--
DROP TABLE IF EXISTS `User story 6 (senaste budet först)`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `User story 6 (senaste budet först)`  AS  select `bids`.`bid_id` AS `bid_id`,`bids`.`bid_item` AS `bid_item`,`bids`.`bid_user` AS `bid_user`,`bids`.`bid_price` AS `bid_price`,`bids`.`bid_time` AS `bid_time` from `bids` where `bids`.`bid_item` = 56 order by `bids`.`bid_time` desc ;
...
--
ALTER TABLE `bids`
  MODIFY `bid_id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=29;

--
-- AUTO_INCREMENT for table `items`
--
ALTER TABLE `items`
  MODIFY `item_id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=101;
